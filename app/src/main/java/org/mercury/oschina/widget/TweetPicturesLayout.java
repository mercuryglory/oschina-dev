...
public class TweetPicturesLayout extends ViewGroup implements View.OnClickListener {
    ...
    public TweetPicturesLayout(Context context, AttributeSet attrs, int defStyleAttr, int
            defStyleRes) {
    ...
    private void init(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (attrs != null) {
            ...
            vSpace = a.getDimensionPixelOffset(R.styleable.TweetPicturesLayout_verticalSpace,
                    vSpace);
            hSpace = a.getDimensionPixelOffset(R.styleable.TweetPicturesLayout_horizontalSpace,
                    hSpace);
            setColumn(a.getInt(R.styleable.TweetPicturesLayout_column, 3));
            setMaxPictureSize(a.getDimensionPixelOffset(R.styleable
                    .TweetPicturesLayout_maxPictureSize, 0));
            a.recycle();
        }
    ...
    public void setImage(String images,String imgBigs) {
        ...
        if (urls.length > 1) {
            for (int i = urls.length - 1; i > 0; i--) {
                urls[i] = commonUrl.concat(urls[i]);
            }
        }
        setImage(urls);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (childCount == 0) {
            ...
            if (!TextUtils.isEmpty(image)) {
                int imageW = 0;
                int imageH = 0;
                imageW = imageW <= 0 ? 100 : 100;
                imageH = imageH <= 0 ? 100 : 100;

                float density = getResources().getDisplayMetrics().density;
                // Get max width and height
                float maxContentW = Math.min(selfWidth - paddingRight - paddingLeft, density *
                        SINGLE_MAX_W);
            ...
            // Measure all child
            final float maxContentWidth = selfWidth - paddingRight - paddingLeft -
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (childCount > 0) {
            ...
            if (childCount == 1) {
                ...
                childView.layout(paddingLeft, paddingTop, paddingLeft + childWidth, paddingTop +
                        childHeight);
            } else {
                int mWidth = r - l;
                int paddingRight = getPaddingRight();
