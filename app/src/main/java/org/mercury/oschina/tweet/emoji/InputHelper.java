 ...
 */
package org.mercury.oschina.tweet.emoji;
...
public class InputHelper {
    ...
    public static Spannable displayUrl(Resources res, CharSequence s) {
        ...
        int urlLength = "<img src=\"http://www.oschina.net/alt=\"3\">".length();
        for (int i = 0; i < str.length(); i +=urlLength) {
            int index = str.indexOf("<img", i);
            int length = str.indexOf(">", index + "<img".length());

            int bound = (int) res.getDimension(R.dimen.space_20);
            try {
                int indexAlt = str.indexOf("alt", index);
                String temp = str.substring(indexAlt + 5, length - 1);
                String emojiStr;
                if (temp.matches("\\d*")) {
                    emojiStr = "[" + temp + "]";
                } else {
                ...
                if (resId > 0) {
                    ...
                    // 封装到ImageSpan中
                    ImageSpan span = new ImageSpan(drawable, ImageSpan.ALIGN_BASELINE); // 对齐基线

                    // 替换指定开始结束位置内容
                    spannable.setSpan(span, index, length + 2,
